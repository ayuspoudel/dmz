...
const HEADER_LINE: &str = "# dotmanz module loader";
const SOURCE_LINE: &str = r#"for f in $HOME/.dotmanz/zsh/*.zsh; do source "$f"; done"#;
...
pub fn run() {
    ...
    // Filter out old dotmanz loader lines
    let mut lines: Vec<String> = original
        .lines()
        .filter(|line| {
            !line.trim().starts_with(HEADER_LINE) &&
            !line.trim().starts_with("for f in $HOME/.dotmanz/zsh")
        })
        .map(|s| s.to_string())
        .collect();
    ...
    match fs::File::create(&zshrc_path) {
        Ok(mut file) => {
            ...
            println!("{YELLOW}ZSH folder:{RESET} {}", std::env::var("HOME").unwrap() + "/.dotmanz/zsh");
